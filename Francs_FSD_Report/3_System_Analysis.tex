...
The technologies chosen for Francs are mature, well-documented, and widely adopted in the industry. Laravel is one of the most popular PHP frameworks, with extensive community support. React is the dominant frontend library for building interactive UIs. Inertia.js is purpose-built for the Laravel + React combination, eliminating the need for a separate REST API layer. Vite 7 provides near-instant HMR in development and optimized production builds. PostgreSQL is used as the relational database engine, offering robust JSON support and performance for production workloads. The entire stack is open-source and free, making the project technically and financially feasible.

\subsection{Economical Feasibility}
Francs is developed entirely using free and open-source technology. Laravel, React, Tailwind CSS, and Vite incur no licensing costs. Email delivery uses Gmail SMTP which is available at no cost for standard usage. Cloud storage for product images uses Cloudinary via the \texttt{cloudinary-laravel} package, which provides a generous free tier. Hosting can be done on platforms such as Railway or Render at minimal cost.
...
\textbf{Laravel 12 (PHP 8.2):} The backend framework providing routing, ORM, authentication, mail, and session management.\\\\
...
\textbf{Laravel Mail:} Email delivery abstraction using SMTP (Gmail) for order confirmation notifications.\\\\
\textbf{Cloudinary:} Cloud-based image storage platform for product and category images via the \texttt{cloudinary-laravel} package.\\\\
\textbf{PostgreSQL:} Relational database for storing all application data.\\\\
